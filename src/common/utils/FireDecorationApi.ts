...
/** This class implements the interaction between the sample a-nd the iTwin.js API.  No user interface. */
export default class FireDecorationApi {
  ...
  public static onViewOpen(type: "Once" | "listener", listener: (viewport: ScreenViewport) => void, scope?: any) {
    console.log('FireDecoratorApi.. onViewOpn');
  ...
  public static getNextTransientId(iModel: IModelConnection): Id64String {
    return iModel.transientIds.getNext();
  }
  ...
  /*
  public static async createFireDecorator(source: Point3d, params: FireParams, viewport?: Viewport): Promise<FireDecorator | undefined> {
    ...
    const fireDecorator = await FireDecorator.create(vp, source, params);
    if (fireDecorator)
  ...
  */
  ...
  public static getAllEmitters(): FireDecorator[] {
    return IModelApp.viewManager.decorators.filter((decorator) => decorator instanceof FireDecorator) as FireDecorator[];
  }
  ...
  public static async queryElements(iModel: IModelConnection, elementsIds: string[]): Promise<Array<{ origin: Point3d, bBox: Range3d }>> {
    ...
    for await (const row of iModel.createQueryReader(query, undefined, { rowFormat: QueryRowFormat.UseJsPropertyNames })) {
      const element = (row.toRow() as { origin: XYAndZ, pitch: number, roll: number, yaw: number, bBoxLow: XYAndZ, bBoxHigh: XYAndZ });
  ...
  public static predefinedParams = new Map<string, FireParams>(
    [
      ...
      [
        ...
        {
          particleNumScale: 0.3,
          sizeRange: Range1d.createXX(0.2, 1),
          transparencyRange: Range1d.createXX(0, 50),
          velocityRange: Range3d.createXYZXYZ(-1, -1, -1, 1, 1, 2),
          accelerationRange: Range3d.createXYZXYZ(-2, -2, -2, 2, 2, 4),
          windVelocity: 0,
          windDirection: Vector3d.createZero(),
          effectRange: Range2d.createXYXY(-0.3, -0.3, 0.3, 0.3),
          height: 1,
          isOverlay: false,
          enableSmoke: true,
          smokeSizeRange: Range1d.createXX(0.1, 0.25),
